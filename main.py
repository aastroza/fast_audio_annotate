...
def select_random_clip() -> Optional[ClipRecord]:
    ...
    pending_clips = [
        ...
        if not clip.human_reviewed and not clip.marked
    ]
    ...
    clip = random.choice(pending_clips)
    print(f"🎯 Selected clip {clip.id} from {clip.audio_path} ({len(pending_clips)} clips remaining)")
